...
{
    public class ServiceMySQL : Service
    {
        ...
        {
            _con = new MySqlConnection(parseConnectionString(_connectionString));
            ...
            for (int i = 0; i < _params.GetLength(0); i++)
            {
                MySqlParameter _param = null;
                string type = _params[i, 1];
                if (Functions.GetTagType(TagType.Boolean).Equals(type))
                    _param = _cmd.Parameters.Add(_params[i, 0], MySqlDbType.Bit);
                else if (Functions.GetTagType(TagType.Float).Equals(type))
                    _param = _cmd.Parameters.Add(_params[i, 0], MySqlDbType.Float);
                else if (Functions.GetTagType(TagType.Integer).Equals(type))
                    _param = _cmd.Parameters.Add(_params[i, 0], MySqlDbType.Int32);
                else if (Functions.GetTagType(TagType.String).Equals(type))
                    _param = _cmd.Parameters.Add(_params[i, 0], MySqlDbType.VarChar);
                else if (Functions.GetTagType(TagType.DateTime).Equals(type))
                    _param = _cmd.Parameters.Add(_params[i, 0], MySqlDbType.DateTime);
                ...
                if (_type == "call")
                {
                    if (_params[i, 4].Equals("inout"))
                    {
                        ...
                        if (_param.MySqlDbType == MySqlDbType.VarChar)
                            _param.Size = 4000;
                    }
