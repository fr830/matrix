...
{
    public class ServiceSQLBase<TCon,TCmd,TDr> : Service
    {
            ...
            for (int i = 0; i < _params.GetLength(0); i++)
            {
                if (_params[i, 4].StartsWith("in"))
                {
                    if (_params[i, 2] == "tag")
                    {
                        ...
                        ((IDbCommand)_cmd).Parameters[_params[i, 0]].Value = (tag.Value != null) ? tag.Value : DBNull.Value;
                    }
                    else
                    {
                        ((IDbCommand)_cmd).Parameters[_params[i, 0]].Value = _params[i, 3];
                    }
                }
...
//                }
            ...
            {
                ((IDbCommand)_cmd).ExecuteNonQuery();
                ...
                for (int i = 0; i < _params.GetLength(0); i++)
                {
                    ...
                    {
                        Tag tag = _server.GetTag(_params[i, 3]);
                        tag.Value = ((IDbCommand)_cmd).Parameters[_params[i, 0]].Value;
                        _server.SetTag(tag);
                    } 
                }
            }
            else
            {
                ...
                IDataReader dr = ((IDbCommand)_cmd).ExecuteReader();
                try
                {
                    if (_type == "scalar")
                    {
