...
{
    public class ServiceOracle : Service
    {
...
//                }
        ...
        {
            _con = new OracleConnection(parseConnectionString(_connectionString));
            ...
            for (int i = 0; i < _params.GetLength(0); i++)
            {
                OracleParameter _param = null;
                string type = _params[i, 1];
                if (Functions.GetTagType(TagType.Boolean).Equals(type))
                    _param = _cmd.Parameters.Add(_params[i, 0], OracleType.Byte);
                else if (Functions.GetTagType(TagType.Float).Equals(type))
                    _param = _cmd.Parameters.Add(_params[i, 0], OracleType.Float);
                else if (Functions.GetTagType(TagType.Integer).Equals(type))
                    _param = _cmd.Parameters.Add(_params[i, 0], OracleType.Int32);
                else if (Functions.GetTagType(TagType.String).Equals(type))
                    _param = _cmd.Parameters.Add(_params[i, 0], OracleType.VarChar);
                else if (Functions.GetTagType(TagType.DateTime).Equals(type))
                    _param = _cmd.Parameters.Add(_params[i, 0], OracleType.DateTime);
                ...
                if (_type == "call")
                {
                    if (_params[i, 4].Equals("inout"))
                    {
                        ...
                        if (_param.OracleType == OracleType.VarChar)
                            _param.Size = 1000;
                    }
